...
// --- EDUPLAN v8 corregida: modo visualización pública ---
function isReadOnlyMode(){
  return new URLSearchParams(window.location.search).get('readonly')==='1' || window.location.pathname.toLowerCase().includes('visualizador');
}
function applyReadOnlyMode(){
  if(!isReadOnlyMode()) return;
  document.body.classList.add('readonly-mode');
  const title = document.getElementById('title');
  const topbar = document.querySelector('.topbar .eyebrow');
  if(topbar) topbar.textContent = 'EDUPLAN · Vista pública de solo visualización';
  const editableIds = ['importBtn','resetBtn','exportBtn','csvBtn','addFindingBtn','addActionBtn','saveFinding','saveAction'];
  editableIds.forEach(id=>{ const el=document.getElementById(id); if(el) el.style.display='none'; });
  document.querySelectorAll('.card-actions, .linkbtn, .danger, dialog, input[type="file"]').forEach(el=>{ el.style.display='none'; });
  document.querySelectorAll('input, textarea, select').forEach(el=>{ if(!el.closest('#reports') && !el.id?.startsWith('dash') && !el.id?.startsWith('report')) el.disabled=true; });
  if(!document.getElementById('readonlyNotice')){
    const notice=document.createElement('div');
    notice.id='readonlyNotice';
    notice.className='notice readonly-banner';
    notice.innerHTML='<b>Vista de solo visualización:</b> este enlace permite revisar dashboard, matriz, acciones y reportes sin modificar datos.';
...
function renderAll(){ initSelects(); renderDashboard(); renderMatrix(); renderActions(); renderReport(); renderConfig(); applyReadOnlyMode(); }
